...
-- swig.need_obj_gc_protect is true if it's dangerous to let scene
-- objects be gced after they've been added to a scene (or to some
-- surface-group).
--
-- swig.obj_gc_protect is false if SWIG offers the "disown" feature
-- (which was supported for Lua starting with SWIG 1.3.35), because
-- the snograw interfaces uses that feature to transfer ownership of
-- objects to the C++ code, avoiding problems with Lua gcing those
-- objects.  If the "disown" feature is not available, then it's not
-- possible to do this, so Lua may gc objects after they've been added
-- to the scene.
--
...
--
swig.need_obj_gc_protect = (not raw.HAVE_SWIG_DISOWN)

local gc_refs = {}

-- Make the keys in GC_REFS weak so that entries don't prevent
-- referring objects from being garbage collected (if nobody else
-- refers to them).
--
setmetatable (gc_refs, { __mode = 'k' })

-- Add a reference from FROM to TO for the garbage-collector to
-- follow.  This is for adding references that the normal gc mechanism
-- cannot deduce by itself, e.g. in userdata objects.
--
function swig.gc_ref (from, to)
